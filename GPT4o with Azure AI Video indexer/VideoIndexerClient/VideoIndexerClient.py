...
#https://api.videoindexer.ai/{location}/Accounts/{accountId}/Videos/{videoId}/Index[?language][&reTranslate][&includeStreamingUrls][&includedInsights][&excludedInsights][&includeSummarizedInsights][&accessToken]
    ...
    def get_video_artifacts(self, video_id, artifact_type:str='KeyframesThumbnails'):
        ...
        self.get_account_async() # if account is not initialized, get it
        url = f'{self.consts.ApiEndpoint}/{self.account["location"]}/Accounts/{self.account["properties"]["accountId"]}/Videos/{video_id}/ArtifactUrl?type={artifact_type}'
    ...
    def download_keyframes(self, video_id, working_directory):
        """
        Download video's artifacts, unzip, copy keyframes and delete zip
        :param video_id:
